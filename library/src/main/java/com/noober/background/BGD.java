...
public class BGD {
    ...
    public static GradientDrawable getDrawable1(TypedArray typedArray) {
        try {
            return _getDrawable(typedArray, true);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private static GradientDrawable _getDrawable(TypedArray typedArray, boolean normal) throws Exception {
        ...
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            ...
            if (attr == R.styleable.background_bg_shape) {
                drawable.setShape(typedArray.getInt(typeIndex, 0));
            } else if (attr == R.styleable.background_bg_solid_color) {
                drawable.setColor(typedArray.getColor(typeIndex, 0));
            } else if (attr == R.styleable.background_bg_corners_radius) {
            ...
            } else if (attr == R.styleable.background_bg_gradient_angle) {
            ...
            } else if (attr == R.styleable.background_bg_gradient_centerColor) {
                centerColor = typedArray.getColor(typeIndex, 0);
            } else if (attr == R.styleable.background_bg_gradient_endColor) {
                endColor = typedArray.getColor(typeIndex, 0);
            } else if (attr == R.styleable.background_bg_gradient_startColor) {
                startColor = typedArray.getColor(typeIndex, 0);
            } else if (attr == R.styleable.background_bg_gradient_gradient_radius) {
                drawable.setGradientRadius(typedArray.getDimension(typeIndex, 0));
            } else if (attr == R.styleable.background_bg_gradient_type) {
        ...
        if (gradientType == LINEAR_GRADIENT &&
                typedArray.hasValue(R.styleable.background_bg_gradient_angle)) {
            gradientAngle %= 360;
    ...
    public static GradientDrawable getDrawable2(TypedArray typedArray) {
        return null;
    }
}
